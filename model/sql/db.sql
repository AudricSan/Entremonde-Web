...
CREATE TABLE `Picture` (
  ...
  `Picture_Link` VARCHAR(10) NULL DEFAULT NULL,
...
CREATE TABLE `Statut` (
  ...
  `Statut_Name` VARCHAR(15) NULL DEFAULT NULL,
...
ALTER TABLE `Article` ADD FOREIGN KEY (Article_Statut) REFERENCES `Statut` (`Statut_ID`);
ALTER TABLE `Article` ADD FOREIGN KEY (Article_tags) REFERENCES `Tags` (`Tag_ID`);
ALTER TABLE `Activity` ADD FOREIGN KEY (Activity_Statut) REFERENCES `Statut` (`Statut_ID`);
...
-- ALTER TABLE `Statut` ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
